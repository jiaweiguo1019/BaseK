...
def read_raw_dataset(business_path, review_path, savepath, pp=None):
...
def reduce_to_k_core(dataset_df, savepath, k_core):
    ...
    user_list = list(map(lambda x: 'item' + sep + x, user_list))
...
def read_reviews(
    business_file, review_file, dirpath, savepath,
    pp=None, drop_dups=False, k_core=None, id_ordered_by_count=True
):

    os.makedirs(savepath, exist_ok=True)
    review_path = os.path.join(dirpath, review_file)
    business_path = os.path.join(dirpath, business_file)

    item_to_cate, raw_dataset_df = \
        read_raw_dataset(business_path, review_path, savepath, pp)
