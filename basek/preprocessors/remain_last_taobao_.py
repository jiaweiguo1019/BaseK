...
def read_reviews(
    ...
    pp=None, drop_dups=False, only_click=False, k_core=None, id_ordered_by_count=False
):
    ...
    processed_raw_dataset_df, user_count, item_count, cate_count, behavior_count = process_raw_dataset(
        raw_dataset_df, savepath, drop_dups=drop_dups, only_click=only_click, k_core=k_core
    )
    ...
    behavior_to_bid, bid_to_behavior = \
        build_entity_id_map(behavior_count, 'behavior', 'bid', savepath, id_ordered_by_count)

    uid_count_path = os.path.join(savepath, 'uid_count.pkl')
    uid_count = {}
    ...
    iid_to_cid_path = os.path.join(savepath, 'iid_to_cid.pkl')
    iid_to_cid = {}
    ...
    for behavior, bid in behavior_to_bid.items():
        if behavior in high_priority_behavior_set:
            bid_to_sample_tempature[bid] = 2
        else:
            bid_to_sample_tempature[bid] = 1
    ...
    all_indices_path = os.path.join(savepath, 'all_indices.pkl')
    ...
    dataset_df_path = os.path.join(savepath, 'dataset_df.pkl')
...
def records_writer(
    ...
    tfrecords_prefix = f'x-max_seq_len_{max_seq_len}'
    if neg_samples:
    ...
    uid_neg_iid_list_path = os.path.join(savepath, f'neg_samples_{neg_samples}-uid_neg_iid_list.pkl')
    uid_neg_cid_list_path = os.path.join(savepath, f'neg_samples_{neg_samples}-uid_neg_cid_list.pkl')
    ...
    def _train_writer():
        train_tfrecords_path = os.path.join(savepath, f'{tfrecords_prefix}-train.tfrecords')
        total_train_samples = 0
        uid_curr_hist_seq_len = defaultdict(int)
        with tf.io.TFRecordWriter(train_tfrecords_path) as writer:
        ...
        print(
            '#' * 132 + '\n'
            + '=' * 32 + f'    writing training samples finished, {total_train_samples} total train samples   '
            + '=' * 32 + '\n'
            + '-' * 4 + f'     train file saved in {train_tfrecords_path}    ' + '-' * 4 + '\n'
            + '#' * 132
        )

    def _test_writer():
        test_tfrecords_path = os.path.join(savepath, f'{tfrecords_prefix}-test.tfrecords')
        total_test_samples = 0
        uid_curr_hist_seq_len = defaultdict(int)
        with tf.io.TFRecordWriter(test_tfrecords_path) as writer:
            for idx, row in tqdm(enumerate(dataset_df.itertuples())):
                _, uid, iid, cid, bid, timestamp = row
                curr_hist_seq_len = uid_curr_hist_seq_len[uid]
                if curr_hist_seq_len != uid_hist_seq_len[uid] - 1:
        ...
        print(
            '#' * 132 + '\n'
            + '=' * 32 + f'    writing test samples finished, {total_test_samples} total test samples   '
            + '=' * 32 + '\n'
            + '-' * 4 + f'     test file saved in {test_tfrecords_path}    ' + '-' * 4 + '\n'
            + '#' * 132
        )

    if write_train:
        train_writer_t = Thread(target=_train_writer)
        train_writer_t.setDaemon(True)
        train_writer_t.start()
    if write_test:
        test_writer_t = Thread(target=_test_writer)
