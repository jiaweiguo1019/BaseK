...
from basek.layers.base import (
    ...
    Concatenate, Dense, Dropout, Embedding, Flatten, Index, Lambda
)

from basek.utils.compute_metrics import ComputeMetrics
...
if __name__ == '__main__':
    ...
    train_file = '/data/project/tfrecords_dataset/seq_len_50-k_core_20-id_ordered_by_count-train.tfrecords'
    test_file = '/data/project/test_tfrecords/seq_len_50-test_drop_hist-k_core_20-id_ordered_by_count-test.tfrecords'
    ...
    test_dataset = test_dataset.padded_batch(16 * BATCH_SIZE).prefetch(-1)
    ...
    def _user_all_embs(uid, hist_iid_seq, hist_cid_seq, hist_seq_len, training):
        ...
        all_embs = BatchNormalization()(all_embs, training=training)
        return all_embs
    ...
    def _item_all_embs(iid, cid, training):
        ...
        item_all_embs = BatchNormalization()(item_all_embs, training=training)
        return item_all_embs

    def _user_hidden(all_embs, training):
        user_hidden_1 = Dense(1024, 'gelu', name='user_hidden_1')(all_embs)
        user_hidden_1 = BatchNormalization()(user_hidden_1, training=training)
        user_hidden_2 = Dense(512, 'gelu', name='user_hidden_2')(user_hidden_1)
        user_hidden_2 = BatchNormalization()(user_hidden_2, training=training)
        user_hidden_3 = Dense(256, 'gelu', name='user_hidden_3')(user_hidden_2)
        user_hidden_3 = BatchNormalization()(user_hidden_3, training=training)
        user_out = Dense(ITEM_EMB_DIM + CATE_EMB_DIM, name='user_out')(user_hidden_3)
        user_out = BatchNormalization()(user_out, training=training)
        return user_out
    ...
    metrics = ComputeMetrics(MATCH_POINTS, './youtubednn')
                    ...
                    if batch_num % 10000 == 0:
                        ...
                        while True:
                            try:
                                ...
                                metrics.add_one_batch(I, ground_truth_iid_seq, hist_iid_seq)
